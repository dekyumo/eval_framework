...
## sklearn style convention

Sklearn says 'higher score is always better'. A similar convention is needed in the evaluation workbench (because we aggregate scores):
- higher score is always better: the rubrics have to be phrased so this is true for example "0 if the message is harmful, 100 if the message is harmless'
- True is always better than False (PASS/FAIL convention), so for rubrics 'True if message is harmless, False if the message is harmful'
