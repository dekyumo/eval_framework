...
def get_governance(repo_path: str, snapshot_id: str) -> dict:
    ...
    return {
        ...
        "all_tags": [{"id": tag.id, "name": tag.name} for tag in tags],
    }


def update_governance(repo_path: str, snapshot_id: str, data: dict) -> dict:
    connection = conn(repo_path)
    ...
    snapshot.governance = GovernanceProfile(
        concern_coverage=str(data.get("concern_coverage") or ""),
        business_case=str(data.get("business_case") or ""),
    )
