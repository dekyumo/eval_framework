...
import { GymRegistry } from '../components/registries/GymRegistry';
import { Button } from '../components/ui/Button';
import { PageContainer, PagePane, BorderedSection } from '../components/ui/PageLayout';

export function Registries() {
  const [activeTab, setActiveTab] = useState<'datasets' | 'tags' | 'rubrics' | 'extractors' | 'gyms'>('extractors');
  ...
  const fetchAll = () => {
    ...
    fetch('/api/registries/gyms').then(res => res.json()).then(data => { if (Array.isArray(data)) setGymsList(data); }).catch(console.error);
  };
    ...
    <PageContainer variant="full" className="flex-col">
      <PagePane variant="sidebar" className="w-full h-full flex flex-col">
        <BorderedSection position="header" className="p-2 gap-2 justify-start">
          {['datasets', 'tags', 'rubrics', 'extractors', 'gyms'].map(tab => (
            <Button 
              key={tab}
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          ...
          {activeTab === 'gyms' && (
            <GymRegistry gyms={gymsList} onFetchAll={fetchAll} />
          )}
        </div>
      </PagePane>
    </PageContainer>
